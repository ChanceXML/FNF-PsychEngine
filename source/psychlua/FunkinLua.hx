...
class FunkinLua {
	...
	public static function androidRequire(L:llua.State):Int {
		...
		try {
			...
			if(code != null) {
				...
				Lua.pushvalue(L, -4); // The execution result or boolean
				Lua.settable(L, -3);
				Lua.pop(L, 2); // Pop package and loaded

				return 1; // Return the result value left on the stack
			} else {
	...
	public function new(scriptName:String) {
		...
		try {
			...
			#if android
			var code:String = null;
			var absPath:String = "storage/emulated/0/Android/data/com.shadowmario.psychengine/files/" + this.scriptName;
			...
			if(code != null && code.length > 0) {
				...
				result = LuaL.dostring(lua, this.scriptName);
			}
			#else
			...
			if(result != 0) {
				...
				luaTrace("Error loading script: " + errMsg, false, false, FlxColor.RED);
				result = null;
			}
