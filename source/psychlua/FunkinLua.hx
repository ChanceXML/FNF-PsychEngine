...
class FunkinLua {
	...
	public static function androidDofile(L:llua.State):Int {
       ...
       try {
        ...
        if(code != null && code.length > 0) {
			...
			// use loadbuffer to prevent SIGSEGV from missing null terminators
            var status:Int = LuaL.loadbuffer(L, code, code.length, absPath);
            if(status != 0) {
            ...
            Lua.pushstring(L, "File not found or empty: " + absPath);
			return Lua.error(L);
        }
	...
	public static function androidRequire(L:llua.State):Int {
		...
		// LuaJIT Fix: Emulate standard package.loaded to prevent double-execution and FFI crashes
		Lua.getglobal(L, "package");
		Lua.getfield(L, -1, "loaded");
		Lua.pushstring(L, path);
		Lua.gettable(L, -2);
		if (Lua.type(L, -1) != Lua.LUA_TNIL) {
			// Module is already loaded, return the cached instance at the top of the stack
			return 1;
		}
		Lua.pop(L, 3); // Pop nil, loaded, and package
		...
		try {
			...
			if(code != null && code.length > 0) {
				...
				// Execute and enforce exactly 1 return value
				...
				// Standard lua behavior: If a module returns nothing (nil), cache it as true
				if (Lua.type(L, -1) == Lua.LUA_TNIL) {
					Lua.pop(L, 1);
					Lua.pushboolean(L, true);
				}

				// Cache the returned result in package.loaded[originalPath]
				Lua.getglobal(L, "package");
				Lua.getfield(L, -1, "loaded");
				Lua.pushstring(L, originalPath);
				Lua.pushvalue(L, -4);
				// The execution result or boolean
				Lua.settable(L, -3);
				Lua.pop(L, 2); // Pop package and loaded

				return 1;
				// Return the result value left on the stack
			} else {
				Lua.pushstring(L, "Error requiring or file empty: " + absPath);
				return Lua.error(L);
			}
		} catch(e:Dynamic) {
			Lua.pushstring(L, "Error requiring: " + absPath);
			return Lua.error(L);
		}
	}
	#end
	...
	public function new(scriptName:String) {
		...
		try {
			...
			if(!pathToCheck.startsWith('/') && !pathToCheck.startsWith('storage')) {
			...
			if(code != null && code.length > 0) {
				// changed dostring to loadbuffer and manual pcall
				result = LuaL.loadbuffer(lua, code, code.length, pathToCheck);
				if(result == 0) {
					result = Lua.pcall(lua, 0, Lua.LUA_MULTRET, 0);
				}
			} else {
                luaTrace("Lua file not found or empty at: " + pathToCheck, false, false, 0xFFFF0000);
				return;
			}
			#else
			...
			if(result != 0) {
				...
				luaTrace("Error loading script: " + errMsg, false, false, 0xFFFF0000);
			}
