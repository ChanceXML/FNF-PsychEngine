...
class FunkinLua {
	...
	#if android
	public static function androidDofile(L:llua.State):Int {
		var path = Lua.tostring(L, 1);
		if (path == null) {
			Lua.pushstring(L, "Error loading file: path is null");
			return Lua.error(L);
		}
       ...
       try {
        ...
        else if(sys.FileSystem.exists(path) && !sys.FileSystem.isDirectory(path)) {
        ...
        if(code != null && code.length > 0) {
			...
			var safePath = (absPath != null) ? absPath : "unknown_file";
			// Added "@" so Lua registers this as a file path in stack traces
            var status:Int = LuaL.loadbuffer(L, code, code.length, "@" + safePath);
            if(status != 0) {
                var err:String = Lua.tostring(L, -1);
				Lua.pop(L, 1);
                Lua.pushstring(L, "Lua compile error: " + (err != null ? err : "unknown"));
                return Lua.error(L);
			}
			...
			if(status != 0) {
                var err:String = Lua.tostring(L, -1);
				Lua.pop(L, 1);
                Lua.pushstring(L, "Lua runtime error: " + (err != null ? err : "unknown"));
                return Lua.error(L);
			}
            ...
            Lua.pushstring(L, "File not found or empty: " + (absPath != null ? absPath : "unknown"));
			return Lua.error(L);
        }

        } catch(e:Dynamic) {
            Lua.pushstring(L, "Exception loading file: " + (absPath != null ? absPath : "unknown"));
			return Lua.error(L);
        }
    }
	...
	public static function androidRequire(L:llua.State):Int {
		var path = Lua.tostring(L, 1);
		if (path == null) {
			Lua.pushstring(L, "Error requiring file: path is null");
			return Lua.error(L);
		}
		...
		try {
			...
			if(code != null && code.length > 0) {
				...
				if(status != 0) {
					var err:String = Lua.tostring(L, -1);
					Lua.pop(L, 1);
					Lua.pushstring(L, "Error compiling file: " + safePath + " - " + (err != null ? err : "unknown"));
					return Lua.error(L);
				}

				status = Lua.pcall(L, 0, 1, 0);
				if(status != 0) {
					var err:String = Lua.tostring(L, -1);
					Lua.pop(L, 1);
					Lua.pushstring(L, "Error running file: " + safePath + " - " + (err != null ? err : "unknown"));
					return Lua.error(L);
				}
				...
				Lua.getglobal(L, "package");
				Lua.getfield(L, -1, "loaded");
				Lua.pushstring(L, originalPath != null ? originalPath : "");
				Lua.pushvalue(L, -4);
				Lua.settable(L, -3);
				Lua.pop(L, 2); 
				...
				// FALLBACK: If file isn't on the SD card, let standard Lua require handle it (for built-in libs like ffi, math)
				Lua.getglobal(L, "original_require");
				Lua.pushstring(L, originalPath != null ? originalPath : "");
				var status = Lua.pcall(L, 1, 1, 0);
				if (status != 0) {
					var err:String = Lua.tostring(L, -1);
					Lua.pop(L, 1);
					Lua.pushstring(L, "Error requiring " + (originalPath != null ? originalPath : "") + ": " + (err != null ? err : "unknown"));
					return Lua.error(L);
				}
				return 1;
			}
		} catch(e:Dynamic) {
			Lua.pushstring(L, "Error requiring: " + (absPath != null ? absPath : "unknown"));
			return Lua.error(L);
		}
	}
	#end
	...
	if (path != null && path.length > 0 && FileSystem.exists(path)) {
		...
		//shitass stuff for epic coders like me B)
		Lua_helper.add_callback(lua, "getObjectOrder", function(obj:String, ?group:String = null) {
			...
			if(leObj != null)
			{
				if(group != null)
				{
					...
					{
						if (Std.isOfType(groupOrArray, Array))
						{
		...
		Lua_helper.add_callback(lua, "addAnimation", function(obj:String, name:String, frames:Dynamic, framerate:Float = 24, loop:Bool = true) {
			return LuaUtils.addAnimByIndices(obj, name, null, frames, framerate, loop);
		});
		Lua_helper.add_callback(lua, "addAnimationByIndices", function(obj:String, name:String, prefix:String, indices:Dynamic, framerate:Float = 24, loop:Bool = false) {
		...
		Lua_helper.add_callback(lua, "getPixelColor", function(obj:String, x:Int, y:Int) {
			if(obj == null) return FlxColor.BLACK;
			var split:Array<String> = obj.split('.');
			var spr:FlxSprite = LuaUtils.getObjectDirectly(split[0]);
			if(split.length > 1) {
				spr = LuaUtils.getVarInArray(LuaUtils.getPropertyLoop(split), split[split.length-1]);
		...
		#if android
		// SAVE ORIGINAL REQUIRE AND DOFILE BEFORE OVERWRITING THEM
		Lua.getglobal(lua, "require");
		Lua.setglobal(lua, "original_require");

		Lua.getglobal(lua, "dofile");
		Lua.setglobal(lua, "original_dofile");

		Lua.pushcfunction(lua, cpp.Callable.fromStaticFunction(androidDofile));
		Lua.setglobal(lua, "dofile");

		Lua.pushcfunction(lua, cpp.Callable.fromStaticFunction(androidRequire));
		...
		try {
			...
			#if android
			var code:String = null;
            var pathToCheck:String = this.scriptName;
			// Logic to prevent double-pathing and fix missing slashes
			if(pathToCheck != null && !pathToCheck.startsWith('/') && !pathToCheck.startsWith('storage')) {
				pathToCheck = "/storage/emulated/0/Android/data/com.shadowmario.psychengine/files/" + this.scriptName;
			}

			if(this.scriptName != null && this.scriptName.length > 0) {
				if(sys.FileSystem.exists(pathToCheck) && !sys.FileSystem.isDirectory(pathToCheck)) {
					code = sys.io.File.getContent(pathToCheck);
				} else if(sys.FileSystem.exists(this.scriptName) && !sys.FileSystem.isDirectory(this.scriptName)) {
					code = sys.io.File.getContent(this.scriptName);
				} else if(openfl.utils.Assets.exists(this.scriptName)) {
					code = openfl.utils.Assets.getText(this.scriptName);
				}
			}

			if(code != null && code.length > 0) {
				var safePath = (pathToCheck != null) ? pathToCheck : "unknown_file";
				// Added "@" so Lua stack traces format cleanly as files
				result = LuaL.loadbuffer(lua, code, code.length, "@" + safePath);
				if(result == 0) {
					result = Lua.pcall(lua, 0, Lua.LUA_MULTRET, 0);
				}
			} else {
                luaTrace("Lua file not found or empty at: " + (pathToCheck != null ? pathToCheck : "null"), false, false, 0xFFFF0000);
				return;
			}
			#else
			var isString:Bool = (scriptName != null) && !FileSystem.exists(scriptName);
			if(!isString) {
				result = LuaL.dofile(lua, scriptName != null ? scriptName : "");
			} else {
				if(scriptName != null && scriptName.length > 0) {
					result = LuaL.dostring(lua, scriptName);
				} else {
					luaTrace("Error: scriptName is null or empty", false, false, 0xFFFF0000);
					return;
				}
			}
			#end
			...
			if(result != 0) {
				var errMsg:String = Lua.tostring(lua, -1);
				luaTrace("Error loading script: " + (errMsg != null ? errMsg : "unknown"), false, false, 0xFFFF0000);
			}
	...
	function findScript(scriptFile:String, ext:String = '.lua')
	{
		...
		#if sys
		try { foundPath = FileSystem.exists(path);
		} catch(e:Dynamic) {}
		try { foundScript = FileSystem.exists(scriptFile); } catch(e:Dynamic) {}
		#end

		if(foundPath || Assets.exists(path, TEXT))
		{
			return path;
		}
		else if(foundScript || Assets.exists(scriptFile, TEXT))
		{
			return scriptFile;
		}
		return null;
	...
	public function initLuaShader(name:String)
	{
		...
		for (folder in foldersToCheck)
		{
			...
			#if sys
			try {
				if (FileSystem.exists(frag) && !FileSystem.isDirectory(frag)) {
					fragCode = File.getContent(frag);
					found = true;
				}
			} catch(e:Dynamic) {}
			#end
			if (fragCode == null && Assets.exists(frag, TEXT)) {
				fragCode = Assets.getText(frag);
				found = true;
			}
			...
			#if sys
			try {
				if (FileSystem.exists(vert) && !FileSystem.isDirectory(vert)) {
					vertCode = File.getContent(vert);
					found = true;
				}
			} catch(e:Dynamic) {}
			#end
			if (vertCode == null && Assets.exists(vert, TEXT)) {
				vertCode = Assets.getText(vert);
				found = true;
			}
