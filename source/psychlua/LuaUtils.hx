...
class LuaUtils
{
	...
	public static function getModSetting(saveTag:String, ?modName:String = null)
	{
		...
		// Mobile Safe fallback if FileSystem fails
		if (!found && openfl.utils.Assets.exists(path, TEXT)) {
			data = openfl.utils.Assets.getText(path);
			found = true;
		}
		...
		if(settings != null && settings.exists(saveTag)) return settings.get(saveTag);
	...
	public static function isMap(variable:Dynamic)
	{
		...
		// Hxcpp 4.3.2 Safe approach instead of checking variable.exists dynamically
		return Std.isOfType(variable, haxe.Constraints.IMap);
	}
